...
$menu_disetujui = $conn->query("SELECT COUNT(*) as total FROM menu WHERE status = 'disetujui'")->fetch_assoc()['total'];
$pengiriman_hari_ini = $conn->query("SELECT COUNT(*) as total FROM pengiriman WHERE tanggal_pengiriman = CURDATE()")->fetch_assoc()['total'];
$keluhan_belum_ditangani = $conn->query("SELECT COUNT(*) as total FROM keluhan k 
    JOIN pengiriman p ON k.pengiriman_id = p.id 
    WHERE p.status = 'diterima'")->fetch_assoc()['total'];
    ...
    <div class="card-hover bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 card-shadow">
        <div class="flex items-center justify-between mb-3 sm:mb-4">
            <div class="w-8 h-8 sm:w-12 sm:h-12 bg-red-100 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
                <i class="fas fa-exclamation-circle text-red-500 text-lg sm:text-2xl"></i>
            </div>
        </div>
        <p class="text-gray-500 text-sm sm:text-sm mb-1.5">Keluhan Belum Ditangani</p>
        <p class="text-2xl sm:text-3xl font-bold text-gray-800"><?php echo $keluhan_belum_ditangani; ?></p>
    </div>
</div>
...
<div class="bg-white rounded-xl sm:rounded-2xl p-6 sm:p-8 card-shadow">
    ...
    <p class="text-gray-600 mb-4">Sebagai Pegawai, Anda dapat:</p>
    <ul class="list-disc list-inside space-y-2 text-gray-700 pl-4">
        <li>Membuat dan mengelola akun (Pegawai, Dokter, Vendor, Sekolah)</li>
        <li>Melihat daftar menu yang dikirimkan oleh Vendor</li>
        <li>Menentukan menu yang sudah disetujui Dokter untuk diberikan ke sekolah tertentu</li>
        <li>Memberikan SP (Surat Peringatan) kepada Vendor</li>
        <li>Melihat laporan dan keluhan</li>
    </ul>
</div>
